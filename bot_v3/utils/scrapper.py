...
def get_products_amazon(results):
    ...
    for single in results[5:]:
        discount = single.find(
            class_="a-row a-size-base a-color-base").find_all('span')[-1]
        if (len(discount) == 0):
            continue
        else:
        ...
        url = "https://www.amazon.in" + \
            single.find(class_="a-link-normal s-no-outline")['href']
...
def get_products_amazon2(results):
    ...
    for single in results:
        discount = single.find(
            class_="a-row a-size-base a-color-base").find_all('span')[-2]
        if (len(discount) == 0):
            continue
        else:
            discount = discount.text
...
def scrap_amazon(category):
    ...
    results = soup.find_all(
        class_='s-card-container s-overflow-hidden aok-relative puis-expand-height puis-include-content-margin puis puis-v32s3p1fquq4652buqesabfpl6j s-latency-cf-section s-card-border')
    # print(soup.prettify)
    if (len(results) == 0):
        results = soup.find_all(
            class_="s-card-container s-overflow-hidden aok-relative puis-include-content-margin puis puis-v20azwp0smsgc01ytmkntf1rk7n s-latency-cf-section s-card-border")
        resp = 2
...
def create_keyword_string(keywords):
    arr = keywords.split(" ")
    _keyword = "+".join(arr)
    # scrap_amazon()
    return _keyword
